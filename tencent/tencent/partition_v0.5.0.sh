#! /bin/sh
partition() {
  tiemout 180 mkfs -t xfs -f /dev/$1
    if [ $? -ne 0 ]; then
      echo "Disk ${disks[$i]} timed out, moving to the next one."
      continue
    fi
  expect -c "
  #!/usr/bin/expect
  ...
  for ((i=0; i<$2; i++))
  do
    timeout 180 mkfs -t xfs -f /dev/$1$4$(($i+1))
    if [ $? -ne 0 ]; then
      echo "Disk ${disks[$i]} timed out, moving to the next one."
      continue
    fi
...
format_hdds() {
  ...
  for i in ${!disks[@]}
  do
    timeout 180 mkfs -t xfs -f /dev/${disks[$i]}
    if [ $? -ne 0 ]; then
      echo "Disk ${disks[$i]} timed out, moving to the next one."
      continue
    fi
